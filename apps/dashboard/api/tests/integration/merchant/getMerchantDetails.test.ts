...
describe("GET /api/merchants/:id", () => {
  ...
  it("should return 200 and merchant details when authenticated", async () => {
  ...
  it("should return 400 when id is not a valid UUID", async () => {
    ...
    const res = await request(app)
      .get("/api/merchants/invalid-uuid")
      .set("Authorization", `Bearer ${accessToken}`)
      .expect(400);
  ...
  it("should return 404 when merchant does not exist", async () => {
    ...
    const res = await request(app)
      .get("/api/merchants/00000000-0000-4000-8000-000000000000")
      .set("Authorization", `Bearer ${accessToken}`)
      .expect(404);
