...
describe("PATCH /api/merchants/:id/activate", () => {
  beforeEach(async () => {
    await cleanup();
  });
  ...
  it("should return 200 and activated merchant when authenticated", async () => {
  ...
  it("should return 400 when id is not a valid UUID", async () => {
    ...
    const res = await request(app)
      .patch("/api/merchants/invalid-uuid/activate")
      .set("Authorization", `Bearer ${accessToken}`)
      .expect(400);
  ...
  it("should return 404 when merchant does not exist", async () => {
    ...
    const res = await request(app)
      .patch("/api/merchants/00000000-0000-4000-8000-000000000000/activate")
      .set("Authorization", `Bearer ${accessToken}`)
      .expect(404);
