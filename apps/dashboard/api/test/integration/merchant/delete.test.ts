...
describe("DELETE /api/merchant/:merchantId", () => {
  ...
  it("returns not found for an unknown merchant", async () => {
    const authUser = await loginUser(await UserFactory.createUser());

    const response = await request(app)
      .delete(`${merchantPath}/${randomUUID()}`)
      .set("Authorization", `Bearer ${authUser.accessToken}`);
  ...
  it("does not delete another user's merchant", async () => {
  ...
  it("rejects an invalid merchant id", async () => {
    ...
    const response = await request(app)
      .delete(`${merchantPath}/invalid-merchant-id`)
      .set("Authorization", `Bearer ${authUser.accessToken}`);
